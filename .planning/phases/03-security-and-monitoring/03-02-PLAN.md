...
must_haves:
  ...
  key_links:
    ...
    - from: "server/internal/api/handler/router.go"
      to: "server/internal/repository/user_repo.go"
      via: "PUT /settings/webhook calls userRepo.UpdateWebhookURL"
      pattern: "userRepo\\.UpdateWebhookURL"
---
...
// After Plan 03-01, socks_pass uses conn.PasswordHash instead of conn.PasswordPlain.
...
In the Connect handler, after looking up the connection, add `bandwidth_limit` and `bandwidth_used` to the payload sent to the tunnel. Note: after Plan 03-01, `socks_pass` uses `conn.PasswordHash` (not PasswordPlain):
```go
body, _ := json.Marshal(map[string]interface{}{
    "client_vpn_ip":   req.VpnIP,
    "device_vpn_ip":   device.VpnIP,
...
<task type="auto">
  <name>Task 2: Offline webhook dispatch and settings API</name>
  <files>
    server/internal/domain/models.go
    server/internal/repository/user_repo.go
    server/internal/repository/device_repo.go
...
```
**Threading userRepo into SetupRouter (REQUIRED for webhook handlers to work):**
The webhook settings handlers need access to `userRepo`. Add `userRepo *repository.UserRepository` as a parameter to `SetupRouter`:
```go
func SetupRouter(
    authService *service.AuthService,
    // ... existing params ...
    syncHandler *SyncHandler,
    userRepo *repository.UserRepository,  // NEW — for webhook settings handlers
) *gin.Engine {
```
Add `"github.com/mobileproxy/server/internal/repository"` to router.go imports.

Update the call site in `server/cmd/api/main.go` (or wherever SetupRouter is called) to pass the existing `userRepo` instance. The `userRepo` is already instantiated in main.go for `authService` — pass the same instance to `SetupRouter`.

The inline handlers above (`GET /settings/webhook`, `PUT /settings/webhook`, `POST /settings/webhook/test`) capture `userRepo` from the `SetupRouter` function parameter via closure — same pattern as the existing `authService` usage in the router.
  </action>
  <verify>
    <automated>cd server && go build ./... 2>&1 | head -20</automated>
  </verify>
  <done>
    ...
    - Settings API endpoints (GET/PUT/POST /settings/webhook) wired with actual userRepo.UpdateWebhookURL call
  </done>
</task>

<task type="auto">
  <name>Task 3: Dashboard monitoring UI (settings page, bandwidth controls)</name>
  <files>
    dashboard/src/lib/api.ts
    dashboard/src/app/settings/page.tsx
    dashboard/src/components/connections/AddConnectionModal.tsx
...
3. Helper functions for byte formatting: `formatBytes(bytes)` → "750 MB", "1.2 GB", etc.
  ...
  <done>
    ...
    - AddConnectionModal has bandwidth limit field (GB input, converted to bytes)
    - ConnectionTable shows usage bar with percentage fill and Reset Usage button
...
8. PUT /settings/webhook handler calls `userRepo.UpdateWebhookURL` (grep confirms `UpdateWebhookURL` in router.go)
9. Settings page exists at /settings with webhook URL management
10. ConnectionTable has usage bar with percentage fill
11. AddConnectionModal has bandwidth limit input
</verification>
