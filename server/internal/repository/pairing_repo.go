...
func (r *PairingCodeRepository) Create(ctx context.Context, pc *domain.PairingCode) error {
	query := `INSERT INTO pairing_codes (id, code, device_auth_token, expires_at, created_by)
		VALUES ($1, $2, $3, $4, $5)`
	_, err := r.db.Pool.Exec(ctx, query, pc.ID, pc.Code, pc.DeviceAuthToken, pc.ExpiresAt, pc.CreatedBy)
	return err
}

func (r *PairingCodeRepository) GetByCode(ctx context.Context, code string) (*domain.PairingCode, error) {
	query := `SELECT id, code, device_auth_token, claimed_by_device_id, claimed_at, expires_at, created_by, created_at
		FROM pairing_codes
	...
	err := r.db.Pool.QueryRow(ctx, query, code).Scan(
		...
		&pc.ClaimedAt, &pc.ExpiresAt, &pc.CreatedBy, &pc.CreatedAt)
	if err != nil {
